...
| **WebRTC** | Low-level `SignalingClient` speaking the `@zero-server/webrtc` wire protocol &mdash; exponential-backoff reconnect, `hello`/`peerId` handshake, coalesced ICE trickle, typed error family (`WebRtcError`, `SignalingError`, `IceError`, `SdpError`, `TurnError`, `E2eeError`); high-level `$.webrtc.join()` room/peer/track surface ships in upcoming releases |
