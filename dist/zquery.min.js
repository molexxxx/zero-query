...
await this.pc.addIceCandidate({candidate});}catch(err){if(this.ignoreOffer)return;this._emit('error',new IceError(err.message||'addIceCandidate failed',{code:'ZQ_WEBRTC_ICE_ADD_FAILED',cause:err,}));}}}{SignalingClient}from'./signaling.js';{Peer}from'./peer.js';{parseSdp,validateSdp,SDP_DIRECTIONS,}from'./sdp.js';{parseCandidate,stringifyCandidate,filterCandidates,isPrivateIp,isLoopbackIp,isLinkLocalIp,isMdnsHostname,CANDIDATE_TYPES,TCP_TYPES,}from'./ice.js';{WebRtcError,SignalingError,IceError,SdpError,TurnError,E2eeError,}from'./errors.js';const webrtc={SignalingClient,Peer,parseSdp,validateSdp,parseCandidate,stringifyCandidate,filterCandidates,isPrivateIp,isLoopbackIp,isLinkLocalIp,isMdnsHostname,WebRtcError,SignalingError,IceError,SdpError,TurnError,E2eeError,async join(){throw new WebRtcError('webrtc.join() is not implemented yet - use SignalingClient directly',{code:'ZQ_WEBRTC_NOT_IMPLEMENTED'});},};function reactive(target,onChange,_path=''){if(typeof target!=='object'||target===null)return target;if(typeof onChange!=='function'){reportError(ErrorCode.REACTIVE_CALLBACK,'reactive() onChange must be a function',{received:typeof onChange});onChange=()=>{};}
