...
const attempt=this._attempts++;const delayMs=Math.min(cfg.capMs,cfg.baseMs*Math.pow(2,attempt));this._emit('reconnect',{attempt:attempt+1,delayMs});this._reconnectTimer=setTimeout(()=>{this._reconnectTimer=null;if(!this.closed)this._open();},delayMs);}}{SignalingClient}from'./signaling.js';{WebRtcError,SignalingError,IceError,SdpError,TurnError,E2eeError,}from'./errors.js';const webrtc={SignalingClient,WebRtcError,SignalingError,IceError,SdpError,TurnError,E2eeError,async join(){throw new WebRtcError('webrtc.join() is not implemented yet - use SignalingClient directly',{code:'ZQ_WEBRTC_NOT_IMPLEMENTED'});},};function reactive(target,onChange,_path=''){if(typeof target!=='object'||target===null)return target;if(typeof onChange!=='function'){reportError(ErrorCode.REACTIVE_CALLBACK,'reactive() onChange must be a function',{received:typeof onChange});onChange=()=>{};}
